...
def dicke_from_ising(J, threshold=1e-6):
    ...
    lams = np.array(
        [vect for vect, val in zip(vects.T, vals) if np.abs(val) >= threshold]
    )
...
def truncate_dicke(ws, lams, M):
    ...
    lams_truncated = np.array(
        [vect for i, vect in enumerate(lams.T) if len(ws) - i <= M]
    ).T
...
def Dk_exact(Jrs, k, N):
    ...
    return (
        Jrs[0]
        + np.sum(Jrs[1:] * 2 * np.cos(k * rs))
        + Jrs[-1] * np.exp(1j * k * (N // 2)).real
    )


def critical_point_recursive_refinement(
    f_order_parameter, critical_parameter, other_args, rounds=2, *, verbose=False
):
    ...
    if rounds == 1:
        ...
        return critical_point_recursive_refinement(
            f_order_parameter,
            new_critical_parameter,
            other_args,
            rounds - 1,
            verbose=verbose,
        )
