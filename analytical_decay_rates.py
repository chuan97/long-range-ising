...
for m, beta in enumerate(betas):
    ...
    for n, J0 in enumerate(J0s):
        ...
        for alpha in alphas:
            ...
            popt, pcov = curve_fit(
                correlation_length_fit,
                np.arange(fit_start, N // 2, 1),
                susc[fit_start : N // 2],
                p0=[1, 1],
            )
        ...
        popt, pcov = curve_fit(
            correlation_decay_fit,
            alphas[: 2 * len(alphas) // 4],
            crit_exp[: 2 * len(alphas) // 4],
            p0=(1, 0),
        )
        rates_of_decay_rates[m, n] = popt[0]

np.savez(
    f"data/analytical_rates_of_decay_rates_{N}_{M}_{len(alphas)}_{len(Ts)}_{len(J0s)}_fixed_large",
    Ts=Ts,
    J0s=J0s,
    rates=rates_of_decay_rates,
)
