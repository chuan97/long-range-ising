...
def uks_f(uks, beta, wz, ws, λs, gs, N):
    ...
    return beta * np.sum(ws * uks**2) - 1 / N * np.sum(
        np.log(2 * np.cosh(0.5 * beta * np.sqrt(wz**2 + 4 * aux**2)))
    )


def u_f(u, beta, wz, w0):
    return beta * w0 * u**2 - np.log(
        2 * np.cosh(0.5 * beta * np.sqrt(wz**2 + 16 * u**2))
    )


def mag_longitudinal(beta, wz, ws, λs, gs, N):
    uks = minimize(
        uks_f, x0=np.array([0.1] * len(ws)), args=(beta, wz, ws, λs, gs, N)
    ).x
    aux = kernel(uks, λs, gs)
    # print(xs)
    return (
        0.5
        * np.tanh(0.5 * beta * np.sqrt(wz**2 + 4 * aux**2))
        * 4
        * aux
        / np.sqrt(wz**2 + 4 * aux**2)
    )
...
def mag_longitudinal_old(beta, wz, ws, λs, gs, N):
    def func(xs):
        aux = kernel(xs, λs, gs)
        return beta * np.sum(ws * xs**2) - 1 / N * np.sum(
            np.log(2 * np.cosh(0.5 * beta * np.sqrt(wz**2 + 4 * aux**2)))
        )
...
def mag_longitudinal_hessian_debug(beta, wz, ws, λs, gs, N):
    ...
    max_H_eigval = (
        u**2
        / e**2
        * (16 * beta**2 - 4 * ws[-1] ** 2 * e**2 * beta**2 - 8 * ws[-1] * beta)
    )
...
def Ys_f(beta, wz, ws, λs, gs, N):
    ...
    return (1 - np.tanh(beta * eps) ** 2) * beta * (aux / eps) ** 2 + np.tanh(
        beta * eps
    ) * (1 / eps - (aux**2 / eps**3))
