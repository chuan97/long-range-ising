...
for i, alpha in enumerate(alphas):
    ...
    for M in Ms:
        ...
        ax.plot(np.arange(1, N - ref_idx), J_truncated[ref_idx, ref_idx+1:] / J[ref_idx, ref_idx+1], c=c, label='M=' + next(labels))
        
        if i == 1:
            axins.plot(np.arange(1, N - ref_idx), J_truncated[ref_idx, ref_idx+1:], c=c)
