...
colors = iter(cmap(np.linspace(0.1, 0.9, len(alphas))))
for alpha in alphas:
    print(1 / alpha)
    bs = []
    for N in Ns:
        Jbase = interactions.powerlaw_pbc(N, alpha)
        _, b = interactions.shift(Jbase, 0.0, return_shift=True)
        bs.append(b)
    bs = np.array(bs)
    ...
    rs = np.arange(1, 1e8)
    hline = -2 * np.sum((-1) ** rs * rs ** (-alpha))
    ax.axhline(hline, c=c, ls="dotted")
...
fig.savefig("plots/bS.pdf", bbox_inches="tight", dpi=300)
